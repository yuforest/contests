...
int main(void){
  ...
  sort(v.begin(), v.end(), [&](int i, int j) {
    long double val1 = (long double)A[i] / ((long double) A[i] + (long double) B[i]);
    long double val2 = (long double)A[j] / ((long double) A[j] + (long double) B[j]);
    debug(val1, val2);
    if (val1 == val2) return i < j;
    return val1 > val2;
  });
  rep(i, N) {
    if (i) cout << " ";
