...
int main() {
  int n; cin >> n;
    ...
    // 1行に3つずつずらして並べていくと3の倍数の時は
    // 連結成分がNになるように並べることができる
    // しかしmod3が0でない時は2回ずれるので、連結成分の数がN+2になってしまう
    // 2つ連結成分を減らすために1行目とn/3行目を入れ替える
    // またn行目とn-n/3行目を入れ替える
    // これで連結成分の数を2つ減らすことができる
    if(n%3) swap(ans[0],ans[n/3-1]), swap(ans[n-1],ans[n-n/3]);
    for(int i = 0; i < n; i++) {
