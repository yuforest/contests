...
int main() {
  ...
  vector<int> ans(n);
  // O(N)
  // ソートして隣接する文字同士のLCPを比較すれば十分であることがわかる
  for(int i=0;i<n-1;i++){
    int p=0;
    // iとi+1の文字列の小さい文字分ループ、ここでLCPを求める
    // 文字数の総和が5*10^5以下なので定数倍になる
    ...
    // i番目とi+1番目の答えを最大値で更新する
    ans[v[i].second] = max(ans[v[i].second],p);
    ans[v[i+1].second] = max(ans[v[i+1].second],p);
  }
  // 出力
  for(int j: ans){
    cout << j << endl;
