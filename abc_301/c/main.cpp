...
bool included(char target) {
  string correct = "atcoder";
  rep(i, correct.size()) {
    if (target == correct[i]) return true;
  }
  return false;
}
...
int main() {
  ...
  fore(x, s_mp) {
    if (x.first == '@') continue;
    if (x.second > t_mp[x.first]) {
      if (!included(x.first)) ok = false;
      t_mp['@'] -= x.second - t_mp[x.first];
    }
  }
  fore(x, t_mp) {
    if (x.first == '@') continue;
    if (x.second > s_mp[x.first]) {
      if (!included(x.first)) ok = false;
      s_mp['@'] -= x.second - s_mp[x.first];
    }
  }
  if (!ok) {
    cout << "No" << endl;
